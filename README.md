...
2. Rename the repo following this convention `BUTLER_[name_of_dataset]`. So for the `p_immigration_shootings` dataset it's `BUTLER_p_immigration_shootings`.
...
3. Change the following variables in the [`get.py`](get.py) file. 
    ...
    Sticking with the `p_immigration_shootings` example that would be:
    ```
...
5. Lastly, set the time that the notebook should run in the [`dataset-butler.yml`](.github/workflows/dataset-butler.yml) file. If you don't wanna think too hard about crontab time formatting here's a [helpful resource](https://crontab.guru/).
    ```
    # schedule: # Uncomment to enable scheduled runs
        # - cron: '0 10 * * *' # Uncomment to enable scheduled runs
    ```

6. Test that it works by clicking the `Actions` button above on the GitHub page. Click `Python application` in the left rail and then click the gray `Run workflow` button in the blue highlighted area.
